...
def detect_white_lane_fillter_package(img):        ###########!!(Trial)
    ...
    lower_white = np.array([0, 0, 0]) #([0, 0, 100])
    upper_white = np.array([255, 255, 200])
...
def start():
    ...
    while not rospy.is_shutdown():
        ...
        lines = cv2.HoughLinesP(masked_image, rho=1, theta=np.pi / 180, threshold=12, minLineLength=50, maxLineGap=7)
