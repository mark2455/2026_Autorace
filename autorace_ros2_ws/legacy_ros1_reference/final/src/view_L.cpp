...
{
  ros::init(argc, argv, "view_L");
  ros::NodeHandle nh;
  ros::Subscriber sub = nh.subscribe<sensor_msgs::PointCloud2>("/filter_L",10,CallbackFunction);
  pub = nh.advertise<visualization_msgs::Marker>("/view_L",10);
  ros::spin();
  return 0;
}
