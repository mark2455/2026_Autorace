...
class Webot_control:
    def __init__(self):
            ...
            #rospy.Subscriber()
            self.sign_id = 0
            self.aruco_distance = 0
            rospy.Subscriber("/fiducial_vertices", FiducialArray, self.comp_img_CB)
            #rospy.Subscriber("/fiducial_vertices", FiducialArray, self.comp_img_CB) #(거리측정용)
            self.sign_id_pub = rospy.Publisher("sign_id", Int32, queue_size=1)
            self.aruco_distance_pub = rospy.Publisher("aruco_distance", Int32, queue_size=1)
    ...
    def comp_img_CB(self,_data):
        ...
        print(f"Aruco_Distance: {self.aruco_distance}")
        

        self.sign_id = _data.fiducials[0].fiducial_id
        self.aruco_distance = _data.fiducials[0].y0 - _data.fiducials[0].y1

        if (self.aruco_distance.pop(0) > 5) :
