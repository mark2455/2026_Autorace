...
import math # to use pi for radian -> degree
...
class line_tracing():
    def __init__(self):  # to set initial value
        ...
        self.img_sub = rospy.Subscriber("usb_camrgb/image_rect_color/compressed", CompressedImage, self.callback)
        self.vel_pub = rospy.Publisher('/cmd_vel', Twist, queue_size=5)

        self.bridge = CvBridge()

        self.mode = True # turn off if there is obstacle on the front
    ...
    def lidar_callback(self, _data): # receive data and change the mode - move / stop
        cnt = 0
    ...
    def roi(self, image):  # we will not gonna use roi because we will refer to the data right in front of us
        ...
        _shape = np.array(
            [[int(0.0 * x), int(y)], [int(0.0 * x), int(0.85 * y)], [int(1.0 * x), int(0.85 * y)],
             [int(1.0 * x), int(y)], [int(0.0 * x), int(y)]])
