...
class Webot_control:
    ...
    def laser_CB(self,msg):
        ...
        for index, value in enumerate(msg.ranges):
            if 150 < abs(self.degrees[index]) and 0 < value <self.safe_range: # 강사님 150 < abs 를 self.detect_degree <abs 로 바꿔봄
                e_stop_degree_list.append(self.degrees[index])
            if self.detect_degree < abs(self.degrees[index]) and 0 < value <self.detect_range: # 강사님 0 < value 대신 self.safe_range < value 로 바꿈
