...
def detect_white_lane_fillter_package(img):        ###########!!(Trial)
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)   #=========================================== img가 아닌 image로 해야지 않나? img 선언필요
...
def start():
    ...
    while not rospy.is_shutdown():
        ...
        img_hsv =cv2.cvtColor(white_img,cv2.COLOR_BGR2HSV)  #BGR2HSV, RGB2HSV 색변환시 이상한 색이므로 , 카메라 색상 기본값이 HSV인 것으로 추정
        ...
        # 변환 전의 좌표
        src = np.float32([[0, 480], [0, 240], [640, 240], [640, 480]])  # 원래좌표 np.float32([[0, 480], [0, 240], [640, 240], [640, 480]])   
        ...
        # 변환 전의 좌표
        # src = np.float32([[0, 480], [237, 303], [403, 303], [640, 480]])  # 원래좌표 np.float32([[0, 480], [0, 240], [640, 240], [640, 480]])   
        ...
        #범위지정                             
        section = np.array([[40, 200],[40, 1000], [600, 1000],[600, 200]])  #원래는  np.array([[40, 200],[40, 1000], [480, 1000],[480, 200]])
        # 마스크 생성
        ...
        lines = cv2.HoughLinesP(masked_image, rho=1, theta=np.pi / 180, threshold=12, minLineLength=25, maxLineGap=7)
        
        #lines = cv2.HoughLinesP(masked_image, rho=1, theta=np.pi / 180, threshold=25, minLineLength=25, maxLineGap=10)
