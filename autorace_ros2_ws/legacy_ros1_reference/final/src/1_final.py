...
def roi(dst):
    ...
    y = int(dst.shape[0])
    #범위지정
    section = np.array([[110, 200],[110, 1000], [530, 1000],[530, 200]]) 
    # 마스크 생성
...
def start():
    ...
    #Steering Type
        if 1==2: # 라바콘이면
            ...
            webot_speed_pub = rospy.Publisher("/commands/motor/speed",Float64,queue_size=1) # node 역할 정하기
            webot_steer_pub = rospy.Publisher("/commands/servo/position",Float64,queue_size=1) # node 역할 정하기
            ...
            prev_angle = 0  # 이전 프레임에서의 조향각                               #=========================================
            # target_speed = 1500  # 목표 속도 설정
            # min_speed = 1000  # 최소 속도 설정
            speed_step = 50  # 속도 조정 단계
            ...
            while not rospy.is_shutdown():
                ...
                if lines is not None:
                    ...
                    steer = 0.5+ angle * 0.02


                    webot_steer_pub.publish(steer)
                ...
                # cv2.imshow("roi_image = masked_image", roi_image)
        ...
        def main():
                        ...
                        rospy.Subscriber("/filter", PointCloud2, webot_control.laser_callback)
