...
ros::Subscriber sub = nh.subscribe<sensor_msgs::PointCloud2>("/filter", 1, callbackFcn);
    ...
    // topic name : /kdtreePC
    // topic type : sensor_msgs::PointCloud2
    pub = nh.advertise<sensor_msgs::PointCloud2>("/distance", 1);
...
//  ros::NodeHandle nh;
  ...
  // PointCloud2 토픽을 구독하는 구독자를 생성합니다.
//  ros::Subscriber sub = nh.subscribe<sensor_msgs::PointCloud2>("/Laser2PointCloud", 1, pointCloudCallback);
