...
//TODO: caller have to pass detect_line_count value
	...
	bool setDetectYOffset(const int detect_y_offset, const int index);
	...
	int getDetectYOffset(const int index) const;
...
protected:
	virtual void resetLeftPoint(const int index) = 0;
	virtual void resetRightPoint(const int index) = 0;

	virtual void updateNextPoint(const int index) = 0;

	double calculateYawError(const int index);
	double calculateLateralError(const int index);
	void calculateOnceDetectTime(const int64 start_time, const int64 finish_time);
	void calculateAvgDetectTime();

	bool detectedOnlyOneLine(const int index) const;

	void visualizeLine(const int index) const;
	virtual void showImg() const;

	int calculateSteerValue(const int center_steer_control_value, const int max_steer_control_value);

	bool haveToResetLeftPoint(const int index) const;
	bool haveToResetRightPoint(const int index) const;

	virtual cv::Point detectLaneCenter(const int index);

	void reservePointReset(const int index);
