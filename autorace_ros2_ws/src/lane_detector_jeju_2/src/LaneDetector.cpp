 ...
 
{
  if(index >= DETECT_LINE_COUNT_) {
	...
	line(resized_img_, cur_right_point_arr_[index] + Point(0, RESIZE_HEIGHT_ / 2), cur_left_point_arr_[index] + Point(0, RESIZE_HEIGHT_ / 2), Scalar(255), 5); //Scalar(0, 255, 0), 5
	line(resized_img_, lane_middle_arr_[index] + Point(0, RESIZE_HEIGHT_ / 2), Point(resized_img_.cols / 2, resized_img_.rows), Scalar(255), 5); //Scalar(0, 0, 255), 5
...
void LaneDetector::showImg() const
{
  ...
  // Mat resized_capston;
  // resize(resized_img_, resized_capston, Size(1280,1280));




	imshow("roi_binary_img_", resized_img_);
	waitKey(1);
}
...
float LaneDetector::getRealSteerAngle() const { return yaw_error_ * yaw_factor_; }
...
void LaneDetector::preprocessImg(const cv::Mat& raw_img)
{
	...
	// cvtToRoiBinaryImg(Point(0, RESIZE_HEIGHT_/2), Size(RESIZE_WIDTH_/2, RESIZE_HEIGHT_/2));
  cvtToRoiBinaryImg(Point(0, RESIZE_HEIGHT_ / 2), Size(RESIZE_WIDTH_, RESIZE_HEIGHT_ / 2));
  // cvtToRoiBinaryImg(Point(0, 0), Size(RESIZE_WIDTH_, RESIZE_HEIGHT_));
  // cvtToRoiBinaryImg(Point(0, 0), Point(1280, 720));
}

//전재범
void LaneDetector::findLanePoints()
{
    int img_width = resized_img_.cols;
    int left_limit_end = img_width * 0.5; // 왼쪽 % 지점
    int right_limit_start = img_width * 0.5; // 오른쪽 % 지점

    for(int index = 0; index < DETECT_LINE_COUNT_; index++) {

        for (int x = 0; x < img_width; ++x) {
            // 중앙 %를 무시하는 조건
            if (x >= left_limit_end && x < right_limit_start) {
                continue; // 중앙 부분을 탐지하지 않음
            }
