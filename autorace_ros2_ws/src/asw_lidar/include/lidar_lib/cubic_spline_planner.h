...
class Spline
{
  private:
    ...
    vecD a, b, c, d, w;  //a,b,c,d : 3차 방정식의 계수
    vecD x, y; //x,y : Waypoint_X and Y
    int nx; //num_x
...
{
  private:
  vecD x, y, s, ds; //x,y : X(s), Y(s)
  Spline sx, sy;

  public:
    Spline2D(vecD x_in, vecD y_in) //x_in : Waypoint_X, y_in : Waypoint_Y
    {
      x = x_in;
      y = y_in;

      s = calc_s(x, y); //x_in : Waypoint_X, y_in : Waypoint_Y

      sx.init(s, x); 
      sy.init(s, y);//X(s)와 Y(s)의 3차방정식 계수 결정
    }

    vecD calc_s(vecD x, vecD y); //x,y로 거리 구하기
...
//input : x[i] - x[i-1]
//output : MatrixXd
MatrixXd Spline::calc_B(vecD h) // get matrix B -> part 4
{
...
//input : s[]
//output : ("the index of first" that value is bigger than "input") -1 
...
//정체는 모르겠지만 무언가의 행렬 공식으로 추정됨. 
//-> 복잡한 계산을 행렬 계산으로 대체
/*
spline 3차 보간
a,b,c,d를 구하는 것이 목표

x(s) = a + b*x + c*x^2 + d*x^3 
a,b,c,d 
input : x_in, y_in
*/
void Spline::init(vecD x_in, vecD y_in) //x_in : S of Spline2D, y_in : Watypoint_X or Waypoint_Y
// calculates the coeffs for splines
{
	x = x_in;
	y = y_in;
	nx = x.size();
	...
	MatrixXd C = A.inverse() * B; // C = A^(-1)*B     C=z-u*c?
...
vecD Spline2D::calc_s(vecD x, vecD y) //x : Waypoint_X, y: Waypoint_Y
//Waypoint_X ,Waypoint_Y를 이용해 Spline2D의 S 만들기
// approximately calculates s along the spline
{
	vecD dx;
...
void Spline2D::calc_position(double &x, double &y, double t) //x: ix, y: iy t : s[i]
{
	x = sx.calc(t);
	y = sy.calc(t);
}
...
// yaw and curvature
// x : Waypoint_X, y : Waypoint_Y, rx :reference_x, ry : reference_y, ryaw : reference Yaw, rk : reference 곡률
// ds : s 생성할 간격
Spline2D calc_spline_course(vecD x, vecD y, vecD &rx, vecD &ry, vecD &ryaw, vecD &rk, double ds)
{
	Spline2D sp(x, y); //Waypoint_x, Waypoint_y 대입하여 X(s), Y(s) 생성
	vecD s;
	...
	while (1)
	{
		...
		s.push_back(sInc);
		sInc = sInc + ds;
	} //생성된 Spline2D 길이만큼의 0.1 간격 생성

	// for(i =0 ; i*ds<sRange;i++)  i*ds = sInc
	//   s[i]=i*ds;
	rx.resize(s.size());
	ry.resize(s.size());
	ryaw.resize(s.size()); //접선 각도
	rk.resize(s.size()); //접선 곡률

	for (int i = 0; i < s.size(); i++)
	{
		double ix, iy;
		sp.calc_position(ix, iy, s[i]); // 0.1 간격으로 s 대입하여 ix, iy 계산
		
		rx[i] = ix;
		ry[i] = iy;
		ryaw[i] = sp.calc_yaw(s[i]);
		rk[i] = sp.calc_curvature(s[i]);
		//reference path의 S에 따른 ix,iy,yaw,curvature 계산
	}
	return sp;
}
